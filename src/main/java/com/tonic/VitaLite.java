...
public class VitaLite {
    ...
    public static void main(String[] args) {
        try {
            ...
            if (Versioning.isRunningFromShadedJar()) {
                ...
                if (!currentVersion.startsWith(liveRlVersion)) {
                    System.out.println(
                        "VitaLite version " +
                            currentVersion +
                            " is out of date. Latest version is " +
                            liveRlVersion +
                            "."
                    );
                    ...
                    System.err.println(
                        "Warning: You are running VitaLite version " +
                            currentVersion +
                ...
                String latestVitaRelease =
                    Versioning.getLatestVitaLiteReleaseTag();
                if (!currentVersion.equals(latestVitaRelease)) {
                    ...
                    Static.getCliArgs().parse(args);
                    SelfUpdate.showUpdateAvailableDialog(
                        currentVersion,
                        latestVitaRelease,
                        true
                    );
            ...
            System.out.println(
                "VitaLite version " + currentVersion + " is up to date."
            );
    ...
    private static String buildFullClasspath() throws URISyntaxException {
        ...
        File sourceLocation = new File(
            VitaLite.class.getProtectionDomain()
                .getCodeSource()
                .getLocation()
                .toURI()
        );
